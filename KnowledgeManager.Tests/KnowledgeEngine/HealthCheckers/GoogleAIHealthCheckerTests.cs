...
public class GoogleAIHealthCheckerTests : IDisposable
{
    private readonly Mock<IOptions<ChatCompleteSettings>> _mockSettings;
    private readonly Mock<ILogger<GoogleAIHealthChecker>> _mockLogger;
    private readonly GoogleAIHealthChecker _healthChecker;
    private readonly ChatCompleteSettings _testSettings;
    private readonly string? _originalApiKey;

    public GoogleAIHealthCheckerTests()
    {
        _mockSettings = new Mock<IOptions<ChatCompleteSettings>>();
        _mockLogger = new Mock<ILogger<GoogleAIHealthChecker>>();
        ...
        _originalApiKey = Environment.GetEnvironmentVariable("GEMINI_API_KEY");

        _healthChecker = new GoogleAIHealthChecker(_mockSettings.Object, _mockLogger.Object);
    }

    public void Dispose()
    {
        // Restore original API key
        Environment.SetEnvironmentVariable("GEMINI_API_KEY", _originalApiKey);
    }
    ...
    {
        // Assert
        Assert.Equal("Google AI", _healthChecker.ComponentName);
    ...
    public async Task CheckHealthAsync_WithMissingApiKey_ShouldReturnMissingApiKeyStatus()
    {
        ...
        // Assert
        Assert.Equal("Google AI", result.ComponentName);
        Assert.Equal("Missing API Key", result.Status);
        Assert.False(result.IsConnected);
        Assert.Contains("Google API key not configured", result.StatusMessage);
    }
    ...
    {
        // Arrange
        Environment.SetEnvironmentVariable("GEMINI_API_KEY", "AIzaSyTest1234567890abcdef1234567890abcdef");
    ...
    {
        // Arrange
        Environment.SetEnvironmentVariable("GEMINI_API_KEY", "AIzaSyTest123");
    ...
    public async Task CheckHealthAsync_WithCustomSettings_ShouldUseConfiguration()
    {
        ...
        Environment.SetEnvironmentVariable("GEMINI_API_KEY", "AIzaSyTest1234567890abcdef");
    ...
    {
        // Arrange
        Environment.SetEnvironmentVariable("GEMINI_API_KEY", "invalid-key-format");
    ...
    {
        // Arrange
        Environment.SetEnvironmentVariable("GEMINI_API_KEY", "not-a-valid-google-key");
    ...
    public void Constructor_WithNullSettings_ShouldCreateInstance()
    {
        ...
        var healthChecker = new GoogleAIHealthChecker(null!, _mockLogger.Object);
        Assert.NotNull(healthChecker);
        Assert.Equal("Google AI", healthChecker.ComponentName);
    }
    ...
    {
        // Act & Assert - Constructor doesn't validate, so no exception thrown
        var healthChecker = new GoogleAIHealthChecker(_mockSettings.Object, null!);
        Assert.NotNull(healthChecker);
        Assert.Equal("Google AI", healthChecker.ComponentName);
    }
    ...
    {
        // Act
        var healthChecker = new GoogleAIHealthChecker(_mockSettings.Object, _mockLogger.Object);
    ...
    {
        // Assert - Google AI should have high priority (low number = high priority)
    ...
    {
        // Assert - Google AI is a main external provider so should be critical
        Assert.True(_healthChecker.IsCriticalComponent);
    }

    #endregion

    #region Google AI-Specific Tests

    [Fact]
    public async Task CheckHealthAsync_WithGoogleApiKeyFormat_ShouldBeRecognized()
    {
        // Arrange - Test with typical Google API key format
        Environment.SetEnvironmentVariable("GEMINI_API_KEY", "AIzaSyDXabc123_def456-ghi789JklMnoPqRsTuVwXyZ");
    ...
    [Fact]
    public async Task CheckHealthAsync_WithGeminiModelConfiguration_ShouldUseCorrectModel()
    {
        // Arrange
        var settingsWithGeminiModel = new ChatCompleteSettings
        {
            SystemPrompt = "Test prompt",
            SystemPromptWithCoding = "Test coding prompt",
            Temperature = 0.7,
            GoogleModel = "gemini-1.5-pro-latest"
        };

        _mockSettings.Setup(x => x.Value).Returns(settingsWithGeminiModel);
        Environment.SetEnvironmentVariable("GEMINI_API_KEY", "AIzaSyTest123");
        ...
        Assert.Equal("Google AI", result.ComponentName);
        Assert.NotEqual("Missing API Key", result.Status);
        // The health checker should attempt to use the configured Gemini model
        Assert.NotNull(result.StatusMessage);
    }
