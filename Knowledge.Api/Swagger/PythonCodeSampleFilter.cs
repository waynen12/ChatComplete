...
public sealed class PythonCodeSampleFilter : IOperationFilter
{
    ...
    {
        if (!string.Equals(ctx.ApiDescription.HttpMethod, "POST", StringComparison.OrdinalIgnoreCase) ||
            !ctx.ApiDescription.RelativePath!.EndsWith("api/chat", StringComparison.OrdinalIgnoreCase))
            return;
...
payload = {""knowledgeId"": ""TrackerSpec"", ""message"": ""Hello""}
