...
public sealed partial class MongoKnowledgeRepository : IKnowledgeRepository
{
    ...
    public MongoKnowledgeRepository(
        IMongoDatabase database,
        ILogger<MongoKnowledgeRepository> log)
    {
        _database = database;
        _log      = log;
    }
        ...
        CancellationToken cancellationToken = default)
    {
       ...
       .ToList();
        ...
        foreach (var name in collectionNames)
        {
            ...
            var docCount = (int)await collection.CountDocumentsAsync(
                FilterDefinition<BsonDocument>.Empty,
                cancellationToken: cancellationToken);

            summaries.Add(new KnowledgeSummaryDto
            {
